...
class Resolution(Generic[RT, CT, KT]):
    ...
    def __init__(
        ...
        # Per-candidate nogoods: when a candidate fails, record the full
        # mapping state so we skip re-trying it with the exact same pins.
    ...
    def _record_candidate_nogood(
        ...
        """Record that this candidate failed with the full current mapping."""
        pins = frozenset(
            (k, id(v)) for k, v in self.state.mapping.items()
        )
        if not pins:
            return
        ...
        self._candidate_nogoods[key].append((pins, cause_criterion))
