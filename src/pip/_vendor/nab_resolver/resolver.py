...
them directly; the supported public API is ``__init__``, ``solve``,
``resolve``, and ``stats``.
...
class Resolver(Generic[PackageType, VersionType]):
    ...
    def resolve(
        ...
        The pins of :meth:`solve`, for callers that do not need the
        dependency graph.
        """
    ...
    def solve(
        ...
        """Resolve requirements and return the pins, the graph, and the roots.
