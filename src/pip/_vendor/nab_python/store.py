...
The store is the one piece of nab-python a host and nab's own fetch
...
:class:`~nab_python.fetch.FetchCoordinator` writes here from its fetcher
thread; :class:`~nab_python.provider.Provider` reads.  A host that supplies
its own :class:`~nab_python.host.FetchPort` writes here instead.
"""
...
if TYPE_CHECKING:
    ...
    from pip._vendor.nab_index.lazy_wheel import RangeOutcome
