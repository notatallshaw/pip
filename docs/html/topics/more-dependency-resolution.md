...
pip's interface to the resolver, which has not yet been included.
...
nab knows nothing about Python packaging. Its own provider supplies that,
and pip supplies the provider with the index and the metadata through a
fetch port backed by pip's finder, factory and preparer. The provider
answers the resolver with:

* `choose_version` - given a package and the range still allowed for it,
  return the version to decide on. This is where pip's finder reaches the
  resolver, and where yanked releases and prereleases are ruled on.
