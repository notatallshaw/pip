...
def test_downloader(
    ...
    with patch.object(Downloader, "_http_get", _http_get_mock):
        if expected_bytes is None:
            ...
            # Make sure the incomplete file is removed. Do not mock os.remove
            # here: the real call is what fails on Windows if pip still holds
            # the file open.
...
def _incomplete_response() -> MockResponse:
    """A response whose body is shorter than the Content-Length it declares."""
...
def test_incomplete_download_is_closed_before_removal(tmpdir: Path) -> None:
    """Windows refuses to delete a file the process still holds open, so the
    output file has to be closed before pip removes an incomplete download."""
...
def test_failed_removal_does_not_mask_incomplete_download(tmpdir: Path) -> None:
    """The download error matters more than a file the cleanup could not delete,
    so a failed removal is swallowed rather than raised."""
    session = PipSession(resume_retries=0)
    link = Link("http://example.com/foo.tgz")
    downloader = Downloader(session, "on")
    ...
    # Nothing could delete it, so the partial file is the accepted cost.
