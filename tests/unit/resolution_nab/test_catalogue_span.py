...
def test_cached_span_stops_at_different_or_unread_metadata():
    ...
    assert versions[1] in span and versions[2] in span
    assert versions[0] not in span and versions[3] not in span
    assert Version("2.5") in span


def test_missing_selected_metadata_does_not_absorb_unknown_neighbors():
    versions = list(map(Version, ("1", "2", "3")))
    span = cached_dependency_span("app", versions[1], versions, {})
    assert versions[1] in span
    assert versions[0] not in span and versions[2] not in span


def test_prerelease_is_a_real_boundary_in_the_catalogue():
    versions = list(map(Version, ("1", "2a1", "2", "3")))
    span = cached_dependency_span("app", versions[2], versions, {})
    assert versions[2] in span
    assert versions[1] not in span and versions[3] not in span
